...
export function GameShell({ game, Board }: { game: GameDefinition; Board: ComponentType<PlayProps> }) {
  ...
  const start = () => { setProgress({ done: 0, total: 1 }); setFeedback({ text: '', correct: true, key: 0 }); setSession(s => s + 1); setStatus('playing'); play('voiceInstruction', game.instruction.th, `game-${game.slug}`); };
