...
/*
  ภาพพื้นหลังที่ตรวจแล้วว่าไม่มีใบหน้ามนุษย์หรือสัตว์เลย จึงไม่ต้องทาทับอะไร
  ผูกกับ sha256 เหมือน CAT_FACE_PATCH ภาพที่เปลี่ยนแม้พิกเซลเดียวต้องตรวจใหม่
  - BGLower.png: เมืองลอยฟ้า เมฆ ดาวเคราะห์ จรวด ดาวเทียม — ตรวจทั้งสี่ส่วนของภาพแล้ว ไม่มีคนหรือสัตว์
*/
const REVIEWED_FACELESS_BG = new Set([
  "ff0c6ddbd035aa66b036fe4d5ed3b993bac5676c779ab4fcbcafc13ff2b19358",
]);
...
for (const job of JOBS) {
  ...
  for (const file of files.sort()) {
    ...
    */
    if (isBackgroundDir && !isPatchTarget && !REVIEWED_FACELESS_BG.has(digest)) {
      throw new Error(
